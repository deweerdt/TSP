...
double get_clean_tick_size(gdouble delta) {
  gint magnitude;
  delta/= 5.0; 
...
gtk_plot3d_autoscale(GtkPlot3D *plot)
{
  ...
  /*
  if ((xmax-xmin)/plot->ax->ticks.step > 10) plot->ax->ticks.step= (xmax-xmin)/10;
  if ((ymax-ymin)/plot->ay->ticks.step > 10) plot->ay->ticks.step= (ymax-ymin)/10;
  if ((zmax-zmin)/plot->az->ticks.step > 10) plot->az->ticks.step= (zmax-zmin)/10;
  */
  plot->ax->ticks.step= get_clean_tick_size(xmax-xmin);
  plot->ay->ticks.step= get_clean_tick_size(ymax-ymin);
  plot->az->ticks.step= get_clean_tick_size(zmax-zmin);
