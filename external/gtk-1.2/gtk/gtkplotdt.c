...
{
  GTK_PLOT_DT_CLASS(GTK_OBJECT(data)->klass)->clear(data);
}

gboolean 
gtk_plot_dt_add_node(GtkPlotDT *data, GtkPlotDTnode node)
{
  return(GTK_PLOT_DT_CLASS(GTK_OBJECT(data)->klass)->add_node(data, node));
}

GtkPlotDTnode * 
gtk_plot_dt_get_node(GtkPlotDT *data, gint idx)
{
  return(GTK_PLOT_DT_CLASS(GTK_OBJECT(data)->klass)->get_node(data, idx));
}

gboolean 
gtk_plot_dt_triangulate(GtkPlotDT *data)
{
  return(GTK_PLOT_DT_CLASS(GTK_OBJECT(data)->klass)->triangulate(data));
}
...
gtk_plot_dt_update_tmpnodes(GtkPlotDT *data)
{
  ...
  data->tmp_nodes= g_new(GtkPlotDTnode, 8);
  data->node_0= -8;
  if (!data->tmp_nodes) return 0;
  memset(data->tmp_nodes,0,sizeof(GtkPlotDTnode)*8);  
...
gtk_plot_dt_triangulate_insert_node(GtkPlotDT *data, GtkPlotDTnode *node)
{
  ...
  gdouble err, min;
  GList *list = NULL, *doomed = NULL, *last = NULL;
  gint num = 0;
  ...
  if (doomed) {
	...
	last= list->next;
	data->triangles= g_list_remove_link(data->triangles, list);
	/* we could append the already doomed to the single element 
	 * to speed things up (no need to search for end of list)
	 * but this would lead to some not-yet-understand problems...
	 */
	doomed= g_list_concat(doomed,list);
	delinquentes++;
#ifdef DELAUNAY_DEBUG
    ...
    while(list){
      t = (GtkPlotDTtriangle *)list->data;
      edges[num].a= t->a; edges[num].b= t->b; num++;
      edges[num].a= t->b; edges[num].b= t->c; num++;
      edges[num].a= t->c; edges[num].b= t->a; num++;
      /* now actually delete the "doomed" triangles,
       * we don't need them any longer:
       */
      doomed= list;
      list= g_list_remove_link(list, doomed);
      g_free(doomed->data);
	...
	if ((edges[j].a==edges[k].a && edges[j].b==edges[k].b) ||
      ...
      if (l==1) 
	gtk_plot_dt_add_triangle(data,node->id,edges[j].a,edges[j].b);
			       ...
			       GtkPlotDTtriangle *t2)
{
  ...
  gint u[3] = {t1->a,t1->b,t1->c}, v[3]= {t2->a,t2->b,t2->c};
  for (i=0; i<4; i++) {
    if (u[0]==v[0] && u[1]==v[2]) {
...
gtk_plot_dt_triangulate_tryquad(GtkPlotDT *data)
{
  ...
  for (xrow_prev= nodes, xrow_next= xrow_sec, x=0, y=0, m=0;
	...
	center->c= x1y0->id;
	center->d= x1y1->id;
	gtk_plot_dt_add_triangle(data,-1-m,x0y0->id,x1y0->id);
	gtk_plot_dt_add_triangle(data,-1-m,x1y0->id,x1y1->id);
	gtk_plot_dt_add_triangle(data,-1-m,x1y1->id,x0y1->id);
	gtk_plot_dt_add_triangle(data,-1-m,x0y1->id,x0y0->id);
	m++;
	y++;
      } else {
	x++; y= 0;
...
void
gtk_plot_dt_set_quatrilateral(GtkPlotDT *dt, gboolean set)
{
