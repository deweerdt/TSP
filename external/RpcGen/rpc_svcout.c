...
static void write_program_signature(definition *def, char *storage);
...
static void write_svc_aux(int nomain);
        ...
        int nomain;
{
	if (inetdflag || pmflag) {
		...
		f_print(fout, "%s int _rpcpmstart%s;", var_type, var_init);
		f_print(fout, "\t\t/* Started by a port monitor ? */\n"); 
		if ((!nomain) && 
		    ((inetdflag) ||
		     ((!tirpcflag) && (pmflag) && (timerflag)))) {
		  /* Used into:
		     - write_inetmost called by this function in case inetdflag
		       && !nomain
		       => inetdflag && !nomain
		     - write_inetd_register in case inetdflag and called by
		       do_registers in case inetdflag || !tirpcflag itself
		       called by s_output in case !nomain
                       => inetdflag && (inetdflag || !tirpcflag) && !nomain
		     - write_timeout_func in case timerflag && (!tirpcflag ||
		       !pmflag) and called by write_svc_aux in case !nomain
		       itself called by this function in case inetdflag ||
		       pmflag
		       => timerflag && (!tirpcflag || !pmflag) && !nomain &&
		         (inetdflag || pmflag)
		     Simplification and taking into account this branch is
		     taken in case (inetdflag || pmflag) gives above test. */
		  f_print(fout, "%s int _rpcfdtype%s;", var_type, var_init);
		  f_print(fout, "\t\t/* Whether Stream or Datagram ? */\n");
		}
		if ((!nomain) && (timerflag)) {
		  /* Used when timerflag is set into write_timeout_func, called
		     by write_svc_aux in case nomain is 0 */
		  f_print(fout, "extern fd_set svc_fdset;\n");
		}
		if (timerflag) {
			f_print(fout, "%s int _rpcsvcdirty%s;", var_type, var_init);
	...
	if (inetdflag) {
	  ...
	  if( tirpcflag ) {
            if (netflag) {
              f_print(fout, "\tregister SVCXPRT *%s = (SVCXPRT *)NULL;\n", TRANSP);
	...
	char *storage;
{
	...
	/* Write dispatcher function signature on top of file to avoid
	   not defined function warning */
	for (l = defined; l != NULL; l = l->next) {
		def = (definition *) l->val;
		if (def->def_kind == DEF_PROGRAM) {
			write_program_signature(def, storage);
		}
	}
	...
	for (vp = def->def.pr.versions; vp != NULL; vp = vp->next) {
		for (proc = vp->procs; proc != NULL; proc = proc->next) {
			...
			if ((proc->arg_num < 2) &&
			  ...
			  f_print(fout, "\t/* Dummy test to avoid unused argp argument warning */\n");
			  f_print(fout, "\tif (argp == argp) {\n\t");
			  dummyUnusedTest = 1;
			}
			f_print(fout, "\treturn(");
			if( Cflag )
			...
			if (dummyUnusedTest) {
			  f_print(fout, "\t} else {\n");
			  sprintf(_errbuf, "unexpected test result (argp not equal to argp)");
			  print_err_message("\t\t");
			  f_print(fout, "\t\texit(1);\n");
			  f_print(fout, "\t}\n");
			}
