...
#define startembeddedcomment(where) (where[0] == '/' && where[1] == '/')

static void remove_ending_spaces(char* str);
...
/*
 * scan expecting any of the 4 given token 
 */
void
...
/*
 * Remove ending spaces of string (modify the string itself).
 */
void
	...
	token *tokp;
{
	...
	tokp->str = (char*) NULL;
	commenting = 0;
	embeddedcommenting = 0;
	for (;;) {
		if (*where == 0) {
			for (;;) {
				if (!fgets(curline, MAXLINESIZE, fin)) {
				  if (embeddedcommenting) {
				    *where = 0;
				    return;
				  } else {
				    tokp->kind = TOK_EOF;
				...
				linenum++;
				if ((commenting) || (embeddedcommenting)) {
					break;
		...
		} else if (embeddedcommenting) {
                        if (startembeddedcomment(where)) {
			  /* Multi-line embedded comment */
			  where += 2;
			  tokp->str = (char*) realloc(tokp->str,
						      strlen(tokp->str) + strlen(where) + 2);
			  strcat(tokp->str, "\n");
			  strcat(tokp->str, where);
			  remove_ending_spaces(tokp->str);
			  *where = 0;
                        } else {
			  /* Embedded comment following by something else
			     returns it with keeping the following part */
			  return;
			}
		} else if (startcomment(where)) {
			where += 2;
			commenting++;
		} else if (startembeddedcomment(where)) {
			embeddedcommenting = 1;
		        where += 2;
			tokp->kind = TOK_COMMENT;
			tokp->str = strdup(where);
			remove_ending_spaces(tokp->str);
			*where = 0;
			/* Do not return know to check for multi line embedded
			   comments */
		} else {
			break;
		}
