...
static void pprocdef(proc_list *proc, version_list *vp, char* addargtype, int server_p, int mode);
	...
	definition *def;
{
	...
	if (streq(decl->type, "bool")) {
		f_print(fout, "\tbool_t %s;", decl->name);
	} else {
		f_print(fout, "\t%s %s;", decl->type, decl->name);
	}
	if (decl->afterComment != (char*) NULL) {
	  f_print(fout, "/*%s */", decl->afterComment);
	}
	f_print(fout, "\n\tunion {\n");
	...
	version_list *start;
{
	...
	abort();
	/* NOTREACHED, just to avoid warning */
	return 0;
}
	...
	for (vers = def->def.pr.versions; vers != NULL; vers = vers->next) {
		...
		for(i=0;i<3;i++){
			...
			for (proc = vers->procs; proc != NULL; proc = proc->next) {
				...
				if (proc->afterComment != (char*) NULL) {
				  f_print(fout, "/*%s */\n", proc->afterComment);
				}
				f_print(fout,"%s",ext);
				if ((i == 0) || (i == 1)) {
				  /* C++ or ANSI C */
				  pprocdef(proc, vers, "CLIENT *" CLNT, 0,i);
				} else {
				  /* Old style C */
				...
				if ((i == 0) || (i == 1)) {
				  /* C++ or ANSI C */
				  pprocdef(proc, vers, "struct svc_req *" RQSTP, 1,i);
				} else {
				  /* Old style C */
	...
	definition *def;
{
	...
	for (l = def->def.en.vals; l != NULL; l = l->next) {
		...
		if (l->afterComment != (char*) NULL) {
		  f_print(fout, " /*%s */", l->afterComment);
		}
		f_print(fout, "\n");
	}
	f_print(fout, "};\n");
...
void
pdeclaration(name, dec, tab, separatorBeforeComment, separatorAfterComment)
	char *name;
	declaration *dec;
	int tab;
        char *separatorBeforeComment;
	char *separatorAfterComment;
{
	...
	f_print(fout, separatorBeforeComment);
	if (dec->afterComment != (char*) NULL) {
	  f_print(fout, " /*%s */", dec->afterComment);
	}
	f_print(fout, separatorAfterComment);
}
